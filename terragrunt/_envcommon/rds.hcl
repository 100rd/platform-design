...
# ---------------------------------------------------------------------------------------------------------------------
# This _envcommon file defines the shared RDS configuration used by all environments.
# It provisions a PostgreSQL 16 instance in the VPC database subnets with encryption at rest,
# managed master credentials, and environment-appropriate HA / backup settings.
#
...
inputs = {
  ...
  # Instance sizing (defined per environment in env.hcl)
  ...
  backup_retention_period = local.env_vars.locals.environment == "prod" ? 30 : 7
  deletion_protection     = local.env_vars.locals.environment == "prod" ? true : false
