...
# ---------------------------------------------------------------------------------------------------------------------
# This _envcommon file defines the shared EKS cluster configuration used by all environments.
# It provisions a managed Kubernetes cluster with IRSA support and a "system" managed node group
# using the terraform-aws-modules/eks/aws registry module.
#
...
inputs = {
  ...
  cluster_endpoint_public_access  = local.env_vars.locals.eks_public_access
  ...
  # Instance types and scaling parameters are defined per environment in env.hcl.
  # ---------------------------------------------------------------------------
