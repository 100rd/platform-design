# ---------------------------------------------------------------------------------------------------------------------
# GPU Analysis EKS Cluster — Catalog Unit
# ---------------------------------------------------------------------------------------------------------------------
# Provisions a dedicated EKS cluster for GPU video analysis workloads (real-time
# sport game temperature map analysis).
#
# Key differences from platform EKS:
#   - Always private endpoint (no public access)
#   - System node group sizing from gpu_analysis_config
#   - Tagged with ClusterRole = "gpu-analysis"
# ---------------------------------------------------------------------------------------------------------------------
...
inputs = {
  ...
  # Always private for GPU analysis clusters — no public access
  ...
  eks_managed_node_groups = {
    system = {
      ...
      instance_types = try(local.gpu_analysis_config.eks_instance_types, ["m6i.large"])
      min_size       = try(local.gpu_analysis_config.eks_min_size, 2)
      max_size       = try(local.gpu_analysis_config.eks_max_size, 3)
      desired_size   = try(local.gpu_analysis_config.eks_desired_size, 2)
