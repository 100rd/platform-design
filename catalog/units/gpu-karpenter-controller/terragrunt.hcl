# ---------------------------------------------------------------------------------------------------------------------
# GPU Analysis Karpenter Controller — Catalog Unit
# ---------------------------------------------------------------------------------------------------------------------
# Deploys the Karpenter Helm chart (controller) for the GPU analysis EKS cluster.
# Depends on GPU analysis EKS cluster and GPU analysis Karpenter IAM resources.
# ---------------------------------------------------------------------------------------------------------------------
...
dependency "karpenter_iam" {
  ...
  mock_outputs = {
    iam_role_arn       = "arn:aws:iam::123456789012:role/mock-gpu-analysis-karpenter-role"
    queue_name         = "mock-gpu-analysis-karpenter-queue"
...
inputs = {
  ...
  controller_replicas = try(local.gpu_analysis_config.karpenter_controller_replicas, 2)
  log_level           = try(local.gpu_analysis_config.karpenter_log_level, "info")
