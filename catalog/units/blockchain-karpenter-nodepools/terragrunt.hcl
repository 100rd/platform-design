# ---------------------------------------------------------------------------------------------------------------------
# Blockchain Karpenter NodePools — Catalog Unit
# ---------------------------------------------------------------------------------------------------------------------
# Deploys Karpenter NodePool and EC2NodeClass CRDs for blockchain HPC workloads.
# Uses the extended karpenter-nodepools module with HPC fields (placement groups,
# AZ pinning, block device overrides).
#
# Dependencies: blockchain-eks, blockchain-karpenter-iam, blockchain-karpenter-controller,
#               blockchain-cilium, placement-group
# ---------------------------------------------------------------------------------------------------------------------
...
dependency "karpenter_controller" {
  config_path = "../blockchain-karpenter-controller"
...
dependency "placement_group" {
  ...
  mock_outputs = {
    placement_group_names = { hpc = "mock-placement-group" }
    placement_group_ids   = { hpc = "pg-00000000000000000" }
  }
...
inputs = {
  ...
  nodepool_configs   = try(local.blockchain_config.karpenter_nodepools, {})

  # Use Bottlerocket for Cilium CNI
  ami_family = try(local.blockchain_config.karpenter_ami_family, "Bottlerocket")
}
